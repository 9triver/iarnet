...
class Actor:
    ...
    def __init__(self, store_client: StoreClient):
        ...
        # 消息通信
        # 发送消息队列（支持 actor.Message 和 component.Message）
        self.send_queue = queue.Queue[actor.Message |
    ...
    def _install_requirements(self, requirements: list[str]) -> bool:
        ...
        logger.info(
            f"Installing {len(requirements)} dependencies: {requirements}")
        try:
    ...
    def handle_function(self, msg: actor.Function) -> bool:
        ...
        if msg.Requirements:
            if not self._install_requirements(list(msg.Requirements)):
                logger.warning(
                    f"Failed to install some dependencies for {msg.Name}, continuing anyway")
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Registered function: {msg.Name}{sig}, expected params: {self.expected_params}")
                return True
            except Exception as e:
    ...
    def _process_invoke_request(self, msg: actor.InvokeRequest):
        ...
        if len(invoke_params) == len(msg.Args):
            logger.info(
                f"All parameters collected, executing function {self.function_name}")
            self._execute_and_respond(invoke_params, runtime_id)
        else:
            logger.error(
                f"Failed to collect all parameters: got {len(invoke_params)}/{len(msg.Args)}")
    ...
    def _execute_and_respond(self, invoke_params: dict[str, Any], runtime_id: str):
        ...
        try:
            ...
            # 执行函数
            logger.info(
                f"Executing function {self.function_name} with params: {list(invoke_params.keys())}")
            ...
            # 编码结果
            store_lang = func.language
            data, is_stream = EncDec.encode(value, store_lang)

            # 保存结果到 Store
            object_ref = self.store_client.save_object(
                data, store_lang, is_stream=is_stream)
            ...
            logger.error(
                f"Function {self.function_name} execution failed: {error_msg}", exc_info=True)
            ...
            try:
                for chunk in self.store_client.iter_stream_chunks(object_id):
                    ...
                    if chunk.Value is None:
                        logger.warning(
                            f"Stream {object_id} chunk value is None, offset={chunk.Offset}")
                        continue
    ...
    def _stream_result_chunks(self, object_id: str, generator: Any, language: common.Language):
        ...
        try:
            ...
            if not self.store_client.save_stream_chunk(eos_chunk):
                raise RuntimeError(
                    f"Failed to save EOS chunk for stream {object_id}")
        except Exception as exc:
    ...
    def _unwrap_component_message(self, component_msg: component.Message) -> Optional[actor.Message]:
        ...
        if not component_msg.HasField("Payload"):
            logger.warning(
                "Component message has PAYLOAD type but Payload field is not set")
            return None
    ...
    def wait_for_function(self, socket: zmq.Socket) -> bool:
        ...
        while True:
            try:
                ...
                if msg is None:
                    logger.warning(
                        f"Received non-PAYLOAD component message: {component_msg.Type}")
                    continue
                ...
                if msg.Type == actor.MessageType.FUNCTION:
                    ...
                    if self.handle_function(msg.Function):
                        ...
                        ack_component_msg = self._wrap_actor_message(
                            ack_actor_msg)
                    ...
                    logger.warning(
                        f"Unexpected message type while waiting for Function: {msg.Type}")
            ...
            try:
                ...
                match msg.Type:
                    case actor.MessageType.INVOKE_REQUEST:
                        logger.info(
                            f"Received INVOKE_REQUEST with {len(msg.InvokeRequest.Args)} args")
    ...
    def run(self, zmq_addr: str, component_id: str = ""):
        ...
        try:
            ...
            send_thread = threading.Thread(
                target=self._send_loop, args=(socket,))
