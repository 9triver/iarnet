...
func TestRemoteDockerResourcePerception(t *testing.T) {
	...
	// 设置当前节点的资源信息（没有 docker provider）
	...
	var _ *provider.Service = remoteDockerProvider

	defer func() {
		if remoteDockerProvider != nil {
			remoteDockerProvider.Close()
		}
	}()

	// 注册远程节点的 Docker Provider
	remoteProviderID := "remote-docker-provider"
	connectReq := &providerpb.ConnectRequest{
		ProviderId: remoteProviderID,
	}
	connectResp, err := remoteDockerProvider.Connect(ctx, connectReq)
	...
	capacityResp, err := remoteDockerProvider.GetCapacity(ctx, capacityReq)
	require.NoError(t, err, "Failed to get remote Docker provider capacity")
	...
	healthResp, err := remoteDockerProvider.HealthCheck(ctx, healthReq)
	require.NoError(t, err, "Failed to get remote Docker provider health check")
	...
	remotePeerNode := &discovery.PeerNode{
		...
		ResourceCapacity: remoteNodeCapacity, // 包含 Docker Provider 的资源容量
		ResourceTags:     remoteResourceTags, // Docker Provider 的资源标签
	...
	// 远程节点: CPU (来自 Docker Provider), Memory (来自 Docker Provider), GPU (来自 Docker Provider)
	...
	assert.Equal(t, expectedTotalCPU, aggregatedCapacity.Total.CPU,
		"Aggregated total CPU should include remote node Docker provider capacity")
	...
	assert.Equal(t, expectedTotalGPU, aggregatedCapacity.Total.GPU,
		"Aggregated total GPU should include remote node Docker provider capacity")
