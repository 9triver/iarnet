...
func NewResponseObject(id string, obj *commonpb.EncodedObject, err error) *Message {
	cmd := &ResponseObject{
		ID:    id,
		Value: obj,
	}
...
func NewReady() *Message {
	return NewMessage(&commonpb.Ready{})
}

func NewAppendData(sessionId string, object *commonpb.EncodedObject) *Message {
	cmd := &AppendData{SessionID: sessionId, Object: object}
	return NewMessage(cmd)
}

func NewAppendPyFunc(name string, params []string, venv string, requirements []string, pickedObj []byte, language commonpb.Language) *Message {
	cmd := &AppendPyFunc{
		Name:          name,
		Params:        params,
		Venv:          venv,
...
// func NewAppendActor(name string, params []string, ref *commonpb.ActorRef) *Message {
// 	cmd := &AppendActor{
// 		Name:   name,
// 		Params: params,
// 		// Ref:    ref,
...
func NewAppendArg(sessionId, instanceId, name, param string, flow *commonpb.ObjectRef, encoded *commonpb.EncodedObject) *Message {
	data := &Data{}
	if flow != nil {
...
func NewAppendArgFromRef(sessionId, instanceId, name, param string, flow *commonpb.ObjectRef) *Message {
	return NewAppendArg(sessionId, instanceId, name, param, flow, nil)
}

func NewAppendArgFromEncoded(sessionId, instanceId, name, param string, encoded *commonpb.EncodedObject) *Message {
	return NewAppendArg(sessionId, instanceId, name, param, nil, encoded)
}

func NewReturnResult(sessionId, instanceId, name string, value *commonpb.ObjectRef, err error) *Message {
	cmd := &ReturnResult{
		SessionID:  sessionId,
		InstanceID: instanceId,
		Name:       name,
